...
记录版本：插件 0.3.0（M1 阶段试玩 + 两轮反馈落地）　DSH 0.1.6-alpha.2　Node v24.15.0
...
> **本轮（第二轮反馈）没有把任何"未测"写成"通过"。** 这一轮是阶段内体验打磨：
> 修掉一个真缺陷（重绘丢用户操作状态）、落实一个设计问题（外部插件能力默认关）、
> 补一个界面缺口（对比页展开配置）、实现一个范围补充（中途输入 = 追加轮次）。
> 受影响的既有条目的证据已**重新跑过并更新**（A15 的盲选脱敏约束扩大到新配置面板、
> A14/A16/A23 在改动后重跑仍全绿），详见各条目下的"第二轮反馈补充"。
