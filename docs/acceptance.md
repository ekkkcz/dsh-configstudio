...
> **M4 这一轮没有新增"通过"，也没有把任何"未测"改写成"通过"。** 它做的是**把已有的结论钉死**：
> - **完整回归一次跑齐**（29 套）：**29 通过 / 0 失败 / 0 跳过**，并给出 **A01–A30 逐条覆盖表** ——
>   **覆盖率 30/30，没有被任何套件覆盖的必需项**（证据：`scripts/m4-full-regression.mjs`，
>   报告里的 `coverage.uncovered` 为空数组）。
> - **A04 从"推断"变成"实测"**：`m4-a04-adapter-probe.mjs` **15/15**，用**两个独立口径**
>   （实例 `/models/resolve` 的真实返回 + DSH 安装里的类型声明）确认
>   **宿主没有表达"温度 / 输出上限是否受支持"的字段**。原表述"适配器没有上报这两个维度"
>   **不准确**，已按实测改写（温度无任何能力字段；输出上限只有 `defaultMaxTokens` 这个**默认值**口径）。
>   结论不变：**A04 保持"部分通过"**，属环境限制。
> - **A18 从"没遇到"变成"扫过全部真实数据确认没遇到"**：`m4-a18-usage-scan.mjs` **9/9**，
>   扫全部 81 个 attempt（**73 个真实**）：**65 个成功且上报了用量、0 个"成功但不上报"**。
>   **A18 保持"未测"**，不用模拟结果顶替。
> - ★ **顺带撞出一个真缺陷并修掉**：失败候选显示 `0 tok` → 违反 A18。
>   证据：`交付区/v0.6.0/截图/03-失败候选显示未上报而不是0tok.png`。

> **M3 这一轮把 8 条推进到"通过"**：A10（输入超限：真实界面手工验证，20/20）、
> A17（受控 CDN 的**真实网络**验证：offline 被 CSP 挡下、cdn 拿到 HTTP 200，13/13）、
> A24（展示包在另一个目录用 file:// 打开，报告可读、作品真的渲染出来）、
> A25（复测包导入另一安装：题目与配方 hash 三方一致、不自动执行、模型重新匹配）、
> A26（路径穿越 / 压缩炸弹 / 超大上传全部被拒，且不在插件目录外留文件）、
> A27（两个包的所有条目扫描 0 命中；用户内容里含路径时如实提示而不改写）、
> A28（连续十轮并停止全部预览：30/30）、
> A30（用**真实模型跑出来的实验**补齐"导出 / 导入"这一段，零模型费用）。
> 剩下的**部分通过 1 条**是 A04（温度的"不受支持"判定：适配器不上报这个维度，界面写"未指定"，
> 不伪造结论 —— 这是**环境限制**，不是没做）；**未测 1 条**是 A18（真实模型不上报用量：
> 两轮真实调用里模型都上报了，没遇到"不上报"的真实情形，只能等它自己出现）。
> **这一轮同样没有把任何"未测"写成"通过"** —— 每一条"通过"都指得出脚本、断言数与证据文件。
...
> **计数口径（M3 结束时）**：A01–A30 共 30 条 = **28 通过 + 1 部分通过 + 1 未测**（0 失败、0 阻塞）。
>
> - **M3** 把 8 条推进到"通过"：A10 / A17 / A24 / A25 / A26 / A27 / A28 / A30。
>   A24–A27 由 `scripts/m3-export-import-check.mjs`（**50/50**，两个干净数据目录的真实往返）
>   与 `scripts/m3-real-pack-check.mjs`（**13/13**，真实模型实验）覆盖；
>   A28 由 `scripts/m3-ten-rounds-check.mjs`（**30/30**）覆盖；
>   A10 由 `scripts/m3-input-limit-check.mjs`（**20/20**）；A17 由 `scripts/m3-cdn-check.mjs`（**13/13**）。
>   本轮新增单测：`tests/zip.test.js` 15 条、`tests/pack.test.js` 13 条、
>   `tests/pack-safety.test.js` 11 条（独立只读核查发现的问题逐条回归）、`tests/packs.api.test.js` 6 条。
> 推进到这里的过程：
> - **M1** 把 A02 / A13 / A14 / A16 / A23 从"未测"推进到"通过"；A15 由"通过（仅模拟断言）"
>   升级为"通过（真实 DSH + 真实模型，并在过程中修掉 3 个真实缺陷）"；
>   A07 补上真实上游 404 的证据；A04 补上两个真实模型不同的档位清单。
> - **用户试玩反馈批次**里 A06（四候选 + 并发 2 排队）取得决定性证据，由"部分通过"升为"通过"。
> - **M2** 再把 6 条推进到"通过"：A03（配方版本 → 历史配方不被覆盖）、A08（超时与取消）、
>   A09（生成中真的 SIGKILL 再起来）、A20 / A21（从 M0 试验记录按 A 编号正式归档）、
>   A29（schema 1 → 2 是一次真实迁移）。
> - 剩下的**部分通过 2 条**：A04（温度是否受支持 —— 适配器不上报这个维度，界面写"未指定"、
>   不伪造结论，属**环境限制**）、A30（导出 / 导入属 M3）。
> - **未测 8 条**：A10（输入超限的真实界面手工验证）、A17（受控 CDN 真实网络）、
>   A18（真实模型不上报用量）、A24–A28（展示包 / 复测包 / 路径穿越 / 私密数据 / 连续十轮，属 M3）。
> **每一步都没有把"未测"改写成"通过"。**
...
#### A30 两个真实模型完整流程
- 状态：**通过**（M3 把"导出 / 导入"这一段补上；两段证据合起来覆盖六个环节）
...
- 实际操作过的完整链路（**全部为真实模型产物**）：
  ...
  6. **历史**：实验列表出现该条目，显示"成功 2 有作品 2 已评价：偏好 A"，
     可按标题搜索到并重新打开。
  7. **导出 / 导入（M3 补齐）**：在这条**真实模型跑出来的实验**上导出复测包与展示包，
     导进另一个干净安装并核对 hash —— **全程零模型调用**（不重新生成，只搬运配置与作品）。
     实测见下面 A24–A27 各条与 `scripts/m3-real-pack-check.mjs`（**13/13**）。
- 证据：`docs/evidence/m1-real-compare.json`、`m1-real-compare-2.json`、`m1-a02.json`、
  `m1-compare-walkthrough-1789717897142.json`、`docs/evidence/m3-real-pack-*.json`；
  截图 `docs/evidence/m1-a15-reveal.png`、`docs/evidence/m3-real-showcase-report.png` 等

#### A10 输入超过大小限制
- 状态：**通过**（M3 补齐：**真实界面**手工路径，不只是接口）
- 脚本：`node scripts/m3-input-limit-check.mjs`，**20 / 20**（零费用：开发服务器 + 模拟模型 + 真实 Chromium）
- 判据与实测（全部走界面，读的是服务端权威数据）：

  | 判据 | 实测 |
  | --- | --- |
  | 题目超限被明确拒绝 | 文案带「50000」「52000」「不会替你截断」 |
  | **没有偷偷发出去** | 模型调用日志 **0 行**；数据目录里 0 个实验、0 个 attempt |
  | **原题没有被截断** | 输入框 `value.length` 仍是 **52000** |
  | 起始 HTML 超限（2MB） | 被拒且文案带上限与当前大小；`value.length` 仍是 2306867 字符 |
  | 接口层口径 | 400 且 `field=prompt` |
  | **对照组**（证明前面的拒绝不是环境问题） | 清空起始 HTML、题目恢复正常后点开始 → 真的跑起来（1 实验 / 2 attempt completed / 2 次调用） |

- 证据：`docs/evidence/m3-input-limit-*.json`

#### A17 外部 CDN 资源不可达
- 状态：**通过**（受控 CDN 的**真实网络**验证）
- 脚本：`node scripts/m3-cdn-check.mjs`，**13 / 13**（真实 Chromium 走产品真实预览通路）

  | 判据 | offline 策略 | cdn 策略 |
  | --- | --- | --- |
  | 对这个域的响应 | **0 个响应**（CSP 拒绝；控制台写明违反的 `script-src` 指令） | **HTTP 200**（真下载到了） |
  | 作品脚本 | `THREE` undefined、`data-three=error`、页面异常被桥接上报 | `THREE` 可用、`data-three=ok` |
  | 作品代码 | 打开前后**逐字节一致**（插件不改代码） | 同 |
  | 截图记录 | 如实带 `pageErrors=1 / failedRequests=1`（不伪造成功） | — |

- 判据说明：Chromium 会把被 CSP 挡下的请求也报成一次 `request`，所以"资源到底下没下来"
  用**有没有 response** 判定，而不是数 request（实测踩过这个坑）。
- 证据：`docs/evidence/m3-cdn-*.json`

#### A24 展示包在干净机器打开
- 状态：**通过**
- 实测（`scripts/m3-export-import-check.mjs`）：把展示包解压到**与数据目录无关的另一个目录**，
  用 `file://` 打开报告：
  - 报告标题正确、题目 / 评价 / "包里有什么"三段都在；
  - **报告主体 0 行脚本**（`<script` 计数为 0），作品走 `<iframe sandbox="allow-scripts">`（不含 `allow-same-origin`）；
  - **作品真的渲染出来了**：iframe 里能读到作品内容；真实作品那一遍用 `document.title` 与包里
    文件的 `<title>` 逐个对上（`scripts/m3-real-pack-check.mjs`）；
  - 打开报告 **0 个外部请求、0 个失败请求**（离线可读）；
  - 报告里没有任何绝对本地路径。
- 口径声明：这是"**同一台机器上的另一个目录 + file://**"，不是真的第二台电脑 —— 如实记录。
- 证据：`docs/evidence/m3-export-import-*.json`、`m3-real-pack-*.json`；
  截图 `docs/evidence/m3-showcase-report.png`、`m3-real-showcase-report.png`

#### A25 复测包导入另一安装
- 状态：**通过**
- 实测（两个**干净数据目录** A/B 各起一个开发服务器，用界面完成导出 → 另一个安装导入）：

  | 判据 | 实测 |
  | --- | --- |
  | 题目 hash 一致 | 源实验 / 包里清单 / 导入后本机 **三方相同**（`d59fdf1b858d…`） |
  | 配方 hash 一致 | 每个候选**三方相同**（`9b33f0b195ff…` / `2307d77d94fc…`） |
  | 配方成为本机独立对象 | 第 1 版、`source=pack-import`，可继续追加版本 |
  | **不自动执行** | 导入前后**模型调用日志行数不变**（0 → 0）；导入的实验 0 个 attempt；artifacts 目录没有多出东西 |
  | **重新匹配模型** | 手工造一个"模型不在这台机器上"的包：检视阶段就点明"本机没有这个模型来源"，候选卡写明"原本用谁、必须重选"，模型下拉框停在"（请选择模型）"占位 |
  | 真实模型那一遍 | 在真实模型跑出的实验上导出 → 干净安装导入：题目与配方 hash 逐个一致、0 次调用（`m3-real-pack-check` 13/13） |

- 证据：`docs/evidence/m3-export-import-*.json`、`m3-real-pack-*.json`；
  单测 `tests/packs.api.test.js`（6 条，含"导入后接着跑"）
...
- 实测：
  ...
  | 构造 | 结果 |
  | --- | --- |
  | 等长改名造出的 `../x.txt` 包 | 400 `bad-name`，并说明"没有在本机写入任何文件、没有执行任何东西" |
  | **声明 1KB、实际解压 128MB** 的包 | 按 `size-mismatch` 拒绝：耗时 < 500ms、`arrayBuffers` 增长 < 16MB（解压上限按声明值封顶） |
  | 4MB 压缩炸弹（比值 > 200:1） | 400 `zip-bomb` |
  | 声明 100MB 的上传 | 400 `too-large`（在读进内存之前就拒绝） |
  | 三种坏包之后 | 插件数据目录之外**没有新增文件**；数据目录里也没有留下半成品 |

- 另有单测把这一类钉住：`tests/zip.test.js`（15 条：穿越 / 盘符 / 保留名 / 反斜杠归一化 /
  归一化后重名 / 加密 / ZIP64 / 多卷 / 不支持的方法 / CRC / 截断 / 白名单 / 上限）
  与 `tests/pack-safety.test.js`（本地头名字与中央目录不一致、条目区间重叠、Windows 语义名字、
  `.js.` 这类绕过可执行黑名单的写法）。
- 证据：`docs/evidence/m3-export-import-*.json`、`tests/zip.test.js`、`tests/pack-safety.test.js`

#### A27 导出包与默认日志不含私密数据
- 状态：**通过**
- 实测（真实数据与模拟数据各一遍）：展示包与复测包的**全部条目**扫描
  "本机绝对路径 / 疑似密钥 / Bearer 令牌 / 带 token 的地址" → **0 命中**；
  复测包里没有完整输入文本（编译后的 userText）与推理全文；导入方的错误字段仍是白名单。
- **一条如实的设计取舍**（这一轮定的口径）：用户**自己的题目与配方正文逐字节照抄、不做脱敏** ——
  改写它们会让题目指纹在导出前后对不上（那正是 A25 要的一致性）。代价是"我的题目里写了本机路径"
  这种情况会把路径带出去，所以导出前会**扫描并如实提示**：
  "题目与输出要求 里 1 处看起来像本机路径或密钥……是你自己的内容，插件不会替你改写，不确定就把对应的勾去掉"。
  实测这条提示会出现（`m3-export-import-check` 的 A27 两条断言）。
- **独立只读核查在这一块抓出两个真缺陷，都已修并回归**（`tests/pack-safety.test.js`）：
  ① 原先"先算 hash 再脱敏、写脱敏后的字节" → **用户自己导出的包会被自己的校验拒收**
  （还指控"包被改过"）；② 脱敏的路径模式套在 HTML 上会把 `</title>` 一起吃掉 → **报告整页空白**、作品被截断。
  现在：正文逐字节照抄，只有我们生成的元数据（pack.json / README / 报告字段）脱敏。
- 证据：`docs/evidence/m3-export-import-*.json`、`m3-real-pack-*.json`、`tests/pack-safety.test.js`

#### A28 连续十轮并停止全部预览
- 状态：**通过**
- 脚本：`node scripts/m3-ten-rounds-check.mjs`，**30 / 30**（全程界面点击：首轮 + 10 次「追加一轮」）

  | 判据 | 实测 |
  | --- | --- |
  | 连续十轮 | 11 轮 × 2 候选 = **22 个 attempt 全 completed**，轮号恰好 1..11（无缺号无重号），22 份正文非空，`runs=[]`，模型调用日志恰好 22 行 |
  | **订阅无重复** | 末轮跑完后 3 秒窗口 /live 请求 1 次 ≤ 首轮同窗口 1 次 ×1.5+2；进行中速率 0.98 → **1.01 次/秒**（上限 3.47）；十一轮窗口最大值恒为 1，不随轮次增长 |
  | 预览资源可回收（对比页） | 只保留**最新一轮**的 2 个 iframe（不是 22 个） |
  | 预览资源可回收（离开页面） | 离开对比页后 iframe **归 0**、`window.__arenaFrames` 悬挂引用也归 0 |
  | 刷新 / 重开 | 刷新后归 0；重新打开恢复 2 个且**都渲染出作品内容**（不是"再也打不开"） |

- **这一条是"先红后绿"的**：脚本第一版实测"离开对比页不回收预览"（`showView()` 只切 `hidden`，
  iframe 还挂在 DOM 上继续跑动画），当时把它记成 `knownGaps` 并留了严格开关；
  产品侧补上 `releasePreviews()` 之后这条成为默认硬断言，脚本里的 `knownGaps` 也改成 `fixedGaps` 留档。
  顺带修掉 `window.__arenaFrames` 只增不减的引用滞留。
- 证据：`docs/evidence/m3-ten-rounds-*.json`
...
- **M4 实测（把推断变成证据）**：`scripts/m4-a04-adapter-probe.mjs` **15/15**，
  用**两个独立口径**交叉验证：

  | 口径 | 做法 | 实测结果 |
  | --- | --- | --- |
  | A 接口实测 | 对着正在跑的实例打 `/models/resolve`，看真实返回的键 | 4 个模型全部解析成功；键集合 = provider / model / contextWindow / **defaultMaxTokens** / inputModalities / reasoningEffort / **reasoningEffortSupported** / availableReasoningEfforts / resolvedAt / note |
  | B 类型声明 | 读 DSH **安装里**的 `.d.ts`（按内容定位到 `export interface LlmResolvedModelInfo` 那一份） | 返回类型**含** `defaultMaxTokens`、**不含** `temperature` |

- **实测结论（原表述已修正）**：原来写"适配器没有上报这两个维度的支持情况"**不准确**，
  准确的说法是**没有上报这两个维度的"支持性"**：
  - **温度**：解析结果里**没有任何**温度相关字段 —— 它只能作为**调用参数**传，不是能力声明；
  - **输出上限**：**有字段但口径不同** —— `defaultMaxTokens` 是"调用方不给 `maxTokens` 时
    采用的**默认上限值**"（实测 `deepseek-official/deepseek-flash` = 256000，另三个模型 = null），
    **不是**"该模型支持 / 不支持输出上限"；
  - 唯一存在的"支持性"判定是**思考档位**（`reasoningEffortSupported` 与
    `availableReasoningEfforts`）—— 那正是本条**已实现并验证**的部分。
- **因此**：界面对温度显示"未指定"、对不支持的思考档位**开始前就拦下**（400 + 列出可选值 +
  不创建 attempt），是**与接口现状一致**的诚实做法，不是界面偷懒。
  这一条缺的是**宿主没有的能力**，属**环境限制**。
- 证据：`tests/api.e2e.test.js`「开始前阻止不受支持的思考档位」；
  `docs/evidence/m1-real-compare.json` 与 `m1-real-compare-2.json` 的 `resolved` 字段；
  `docs/evidence/m4-a04-adapter-probe-*.json`（含两个口径的原始键集合与类型声明片段）
...
| ID | 场景 | 计划 |
| --- | --- | --- |
| A18 | 模型不返回 Token | **M4 已扫过全部真实数据，仍未遇到**：`scripts/m4-a18-usage-scan.mjs` **9/9**，扫全部 81 个 attempt（**73 个真实**）—— **65 个成功且上报了用量、0 个"成功但不上报"**；其余是失败 / 取消 / 超时（本来就不会有用量）。功能本身已实现并验证（缺失字段存 null、界面显示"未上报"），**但这一条只能等真实模型自己不上报才算测到**，不能拿模拟结果顶替 —— 所以**保持未测** |

> ★ **M4 在扫这一条时撞出一个真缺陷并修掉了**（属于 A18 的"已实现"部分的缺陷，不是未测的那部分）：
> 失败的候选（额度不足 / 鉴权失败 / 上游 500）在对比页显示
> `输入 0 tok / 输出 0 tok / 合计 0 tok / 总速度 0.0 tok/s`，**违反 A18**"不显示零 Token 零费用"。
>
> **根因不在界面**：宿主 `TokenUsage` 把 `inputTokens` / `outputTokens` 声明成**必填 number**
> （类型上就没有 null）。一次没有成功收尾的调用根本没有用量可报，适配器只能填 0 ——
> 那个 0 是"类型不允许表达未知"的产物，不是"模型说用了 0 个 token"。
>
> **修法**：新增 `src/core/usage.js` 作为**唯一一份**口径，两处调用 ——
> 写入侧 `core/runner.js` 管新数据；**读取侧 `api.js` 的 `describeAttempt` 把历史数据与
> 导入包里的旧记录一并修正**（所以不用重跑旧实验）。成功收尾的调用**一个字都不改**；
> 失败但有非 0 用量的照实保留。
>
> **证据**：`tests/runner.test.js` 新增 8 条（四种组合）；
> 截图 `交付区/v0.6.0/截图/03-失败候选显示未上报而不是0tok.png`（修后实测同一份旧数据显示"未上报"）；
> 扫描里的回归断言"没有成功收尾的真实 attempt 不再显示全 0 用量"（修复前 4 例，修后 0 例）。

> **M3 把 A10 / A17 / A24 / A25 / A26 / A27 / A28 从这张表里移走了** —— 它们现在都有按 A 编号
> 写成的正式小节、脚本、断言数与证据文件（见上面各自的条目）。
> M2 移走的是 A09 / A20 / A21 / A29。这张表里**只留真的没测的**。
