...
**修复方向**（供下一次对话选，倾向第 1 个）：
1. **增量更新**：`renderRun()` 只更新会变的部分（状态文本、实时正文、用量），
   卡片骨架只在候选集合变化时重建。这是根治，且能顺带修掉滚动位置问题。
2. 轻量兜底：重建前记住每个 `details` 的 `open` 状态与滚动位置，重建后恢复。
   改动小，但仍是全量重建，滚动/选中/焦点依然会闪。
...
- 是否泛化：只做"提示词优化"这一个开关，还是做成"外部插件能力"的通用清单？
  ...
  "新增抽象必须对应当前需求"）。
- 默认值：探测到就默认开（现状）还是默认关、由用户主动开？
  用户这句话的倾向听起来是**默认关 / 或至少要显式确认**，实现前值得跟用户确认一句。

### 反馈 3：中途输入提示词（**与 F02 有冲突，需先定设计**）
...
**实现顺序建议**：先修反馈 2（明确缺陷、根因已定位、改动可控），
再做反馈 1（用户已明确要求），最后处理反馈 3（需要先和用户确认取哪个方案）。
