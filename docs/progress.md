...
| **第二轮反馈：追加轮次** | `node scripts/m2-rounds-walkthrough.mjs` | **14/14 通过**（零费用，模拟模型） |
...
| 3 | "中途最好也可以让用户自己输入提示词啥的，中途输入的时候最好也能加插件" | 范围补充 | 用户拍板**方案 1 追加轮次**：中途输入 = 新的一轮 attempt，原轮完整保留；F02 未改写 | `tests/rounds.test.js` 6/6、`m2-rounds-walkthrough` 14/14 |
...
**本项的实测断言**（`tests/rounds.test.js` 6/6，`scripts/m2-rounds-walkthrough.mjs` 14/14）：
