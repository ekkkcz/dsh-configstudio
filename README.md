...
ConfigStudio 是一个 **DSH（DeepSeek Harness）Web 插件**，让你自定义生成配置与对照方式，直观看清不同选择带来的效果差异。
将同一道题交给 2–4 套自定义配置生成，在同一视口下并排体验产物，隐藏配置身份进行盲选，
再揭晓、保存配方，或导出给别人查看与复测。

**比较什么，由你决定。** 每个候选都可以独立设置模型、系统提示词、提示词片段、温度、输出上限与思考档位：

- **同模型，不同配置**：看看提示词、参数或思考档位的变化带来什么效果。
- **不同模型，相同设置**：在各模型支持的范围内保持其他设置一致，对照模型表现。
- **自由组合**：为每个候选搭配不同模型与配置，比较整套方案是否适合你的任务。

你可以只改变一项变量，也可以同时调整多项设置；对照方式由你选择，具体可用项以模型和 DSH 的支持情况为准。

当前版本 **`0.7.0`**，支持生成与预览**单文件 HTML**，适合交互原型、动画和创意页面等任务。
参考图输入、多次采样、交互后截图等能力尚未提供，详见文末「已知限制」。

---

## 一次典型使用长什么样

用一句话说：**你出一道题，让几套配置分别作答，再把结果放在一起体验。**
...
两个单文件 HTML → 并排放在同一个逻辑视口里，可以点、可以玩、可以切手机/桌面视口
                 ↓  点「隐藏配置身份」
隐藏模型与配方信息，凭作品本身选「偏好 A / 偏好 B / 平局 / 无法判断」
                 ↓  点「揭晓」+「导出」
导出展示包供人查看，或导出复测包，让对方用自己的 DSH 配置重新生成
```

**它擅长回答的问题**：

- "同一道题，这两个模型谁做得更合我意？"（匿名对比，减少品牌偏见）
- "同一个模型，换系统提示词、温度或思考档位，效果有什么不同？"（多套配置并排体验）
- "我这个提示词改了一版，比上一版好了吗？"（配方版本只追加，历史可回看）
- "我这套配置不错，发给你试试。"（导出复测包，对方导入后**使用自己 DSH 中可用的模型服务**重跑）
- "上周那个对比我选了谁？为什么？"（历史可搜索、可按评价筛选）

当前版本的能力范围见下方「它不做什么」与文末「已知限制」。
...
| **至少一个可用的模型服务已在 DSH 中配好** | 同一个模型即可对照多套提示词或参数配置；比较不同模型时，再配置相应服务。 |
...
1. **你已经有至少一个可用的模型服务**。插件使用 DSH 中已有的模型配置
   （在 DSH 里 `/models` 能看到、并且能实际调用）。可以选择不同模型，也可以用同一模型搭配不同提示词或参数。
...
3. 在两张候选卡上分别选择模型，并设置要对照的配置。可以选不同模型，也可以选择同一模型，仅调整系统提示词、温度、输出上限或思考档位。
4. 点「**开始生成**」。运行面板会显示排队 / 生成中 / 已完成 / 失败，**实时**能看到内容在增长。
   - 这一轮的花费就是"每候选一次模型调用"。**推理档位开高会明显更贵更慢**（见下面的费用一节）。
5. 两个都完成后点「**进入对比**」：切视口、点「隐藏配置身份」、选偏好并保存、导出。

**第一次建议**：两张候选卡选择同一模型，保持其他设置一致，只改变一项提示词或参数，
先直观看看这项变化对结果的影响。单次生成存在随机性，可以再跑几轮观察结果是否一致。
